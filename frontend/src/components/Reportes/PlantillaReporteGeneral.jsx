...
const PlantillaReporteGeneral = ({ data }) => {
  ...
  // 2. Integración: Alimentadoras + ORION en el mismo grupo
  // Cualquier registro que sea RA... o ORION... se tratará como Alimentadora
  const alimentadoras = data.filter(item => 
    item.ruta.startsWith('RA') || item.ruta.startsWith('ORION')
  );

  // Cálculos automáticos (funcionan igual para cualquier número de elementos)
            ...
            <tbody>
              ...
              {troncales.map((item, i) => (
                <tr key={item.ruta}>
                    ...
                    <td rowSpan={troncales.length} className="td-operacion-group">
                      <div className="operacion-col">
                        <div className="operacion-individual">
                          {troncales.map(r => <span key={r.ruta} className="ind-num">{r.en_operacion}</span>)}
                        </div>
                  ...
                  {i === 0 && (
                    <td rowSpan={troncales.length} className="td-big-negro">{troncalTotal}</td>
                  )}
                </tr>
              ))}

              {/* Bloque ALIMENTADORAS + ORION */}
                ...
                <tr key={item.ruta}>
                  ...
                  {i === 0 && (
                    <td rowSpan={alimentadoras.length} className="td-operacion-group">
                      <div className="operacion-col">
                        <div className="operacion-individual">
                          {alimentadoras.map(r => <span key={r.ruta} className="ind-num">{r.en_operacion}</span>)}
                        </div>
                  ...
                  {i === 0 && (
                    <td rowSpan={alimentadoras.length} className="td-big-negro">{alimentadoraTotal}</td>
                  )}
                </tr>
              ))}
            </tbody>
